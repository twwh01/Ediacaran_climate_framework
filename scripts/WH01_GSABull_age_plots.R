...
## set directory paths ----
dir_data <- file.path("..", "Manuscripts & Writing", "MS_WongHearing_etal_Ediacaran_Biosphere_Climate", "GSABull_submission")
dir_plots <- file.path(dir_data, "figures")
...
infile_deposits <- file.path(dir_data, infile_deposits_name)
# infile_palaeo <- file.path(dir_data, infile_palaeo_name)
...
data_tables_1_3 <- data_deposits_scores %>%
  dplyr::select(
    deposit_name, 
...
# save table to workbook
data_deposits_wb$add_worksheet("Table_3")
data_deposits_wb$add_data(sheet = "Table_3", x = data_table_3)

## save workbook ----
openxlsx2::wb_save(
  data_deposits_wb,
  file = file.path(dir_data, paste0(outdate, "_", infile_deposits_name))
)
...
# figures 2, S2, and S3: deposit correlation and scores ----
## sort data ----
...
plot_figS2_radiometric_constraints_deposit <- ggplot() +
  ...
  scale_colour_viridis_d(option = "turbo", begin = 0, end = 0.9, name = "Constraint:") +
  scale_fill_viridis_d(option = "turbo", begin = 0, end = 0.9, name = "Constraint:")
...
if(isTRUE(plot_save)) {
  png(
    file = file.path(
      ...
      paste0(
        outdate,
        "_fig_S2_radiometric_constraints_distribution_deposits.png"
      )
    ),
    width = 158.7,
    height = 170,
...
if(isTRUE(plot_save)) {
  png(
    file = file.path(
      ...
      paste0(
        outdate,
        "_fig_S3_radiometric_constraints_distribution_craton.png"
      )
    ),
    width = 158.7,
    height = 130,
